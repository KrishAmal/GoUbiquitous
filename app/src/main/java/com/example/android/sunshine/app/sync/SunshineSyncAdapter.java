...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter
    ...
    public void onConnected(@Nullable Bundle bundle) {
        ...
        Asset asset=null;
        Bitmap largeIcon;
        Log.d(LOG_TAG, " WEAR : connected !");

        String temp="";
        String locationQuery_wear = Utility.getPreferredLocation(getContext());
        Uri weatherUri_wear = WeatherContract.WeatherEntry.buildWeatherLocationWithDate(locationQuery_wear, System.currentTimeMillis());
        Cursor cursor_wear = getContext().getContentResolver().query(weatherUri_wear, NOTIFY_WEATHER_PROJECTION, null, null, null);

        if (cursor_wear.moveToFirst()) {

            String high = Utility.formatTemperature(getContext(), cursor_wear.getDouble(INDEX_MAX_TEMP));
            String low = Utility.formatTemperature(getContext(), cursor_wear.getDouble(INDEX_MIN_TEMP));

            int weatherId_wear = cursor_wear.getInt(INDEX_WEATHER_ID);
            Resources resources_wear =getContext().getResources();
            int artResourceId_wear = Utility.getArtResourceForWeatherCondition(weatherId_wear);

            largeIcon = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(resources_wear,
                    artResourceId_wear),70,70,true);

            asset=createAssetFromBitmap(largeIcon);
            temp=high+" "+low;
            Log.d(LOG_TAG, " WEAR : Temperature="+temp);
        ...
        if(asset!=null)
            putDataMapReq.getDataMap().putAsset("com.sunshine.icon",asset);
        putDataMapReq.getDataMap().putString("com.sunshine.temp",temp);
